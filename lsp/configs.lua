...
function configs.__newindex(t, config_name, config_def)
  if config_def.default_config.deprecate then
    vim.deprecate(config_name, config_def.default_config.deprecate.to, config_def.default_config.deprecate.version, 'lspconfig', false)
  end
  ...
  function M.setup(user_config)
    ...
    if config.autostart == true then
      local event_conf = config.filetypes and { event = 'FileType', pattern = config.filetypes } or { event = 'BufReadPost' }
      api.nvim_create_autocmd(event_conf.event, {
        ...
        group = lsp_group,
        desc = string.format('Checks whether server %s should start a new instance or attach to an existing one.', config.name),
      })
    end
      ...
      async.run(function()
        ...
        if root_dir then
          api.nvim_create_autocmd('BufReadPost', {
            ...
            group = lsp_group,
            desc = string.format('Checks whether server %s should attach to a newly opened buffer inside workspace %q.', config.name, root_dir),
          })
    ...
    local manager = require('./manager.lua').new(config, make_config)
